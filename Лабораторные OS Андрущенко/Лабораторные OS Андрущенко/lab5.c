...
int main(int argc, char *argv[]){
	...
	Link foo = createNode("foo", 0, S_IFDIR | 0665);//создаем виртуальную папку foo
	Link bar = createNode("bar", 0, S_IFDIR | 0777);//создаем виртуальную папку bar
	Link bin = createNode("bin", 0, S_IFDIR | 0755);//создаем виртуальную папку bin
	Link baz = createNode("baz", 0, S_IFDIR | 0744);//создаем виртуальную папку baz
	addNode(tree, foo); //Добавляем виртуальную папку foo в виртуальную корневую папку
	addNode(tree, bin);//Добавляем виртуальную папку bin в папку bar
	addNode(bin, bar);//Добавляем виртуальную папку bar в виртуальную корневую папку
	addNode(bin, baz);//Добавляем виртуальную папку baz в папку bar
	Link readme = createNode("readme.txt", "Student Ярослав Андрущенко, 16150001\r\n", S_IFREG | 0444);//Создаем виртуальный файл readme.txt

	struct stat cat_stat;//Объявляем переменную cat_stat
	stat("/bin/cat", &cat_stat);//Получаем статистику реального файла /bin/cat
	size_t len = cat_stat.st_size; //Копируем размер файла из cat_stat.st_size в len
	FILE *f;//Объявляем переменную - ссылку на файл
	Link cat = createNode("cat", (unsigned char *) malloc(len), S_IFREG | 0677);//Создаем виртуальный файл cat, в качестве содержимого передаем указатель на свободную память с размером len
	f = fopen("/bin/cat", "r");//Открываем реальный файл и записываем ссылку на него в f
	fread(cat->content, len, 1, f);//Считываем содержимое из реального файла f в содержимое виртуального файла cat
	fclose(f);//Закрываем файл f

	addNode(baz, readme); //Добавляем виртуальный файл readme.txt в виртуальную папку baz
	addNode(baz, cat);//Добавляем виртуальныйы файл cat виртуальную папку baz
	Link example = createNode("example", "Hello world", S_IFREG | 0200);//Создаем виртуальный файл example
	addNode(baz, example);//Добавляем виртуальный файл example в виртуальную папку baz
	char testtxt_str[2] = "";//Создаем массив символов длиной 2
	for (int i=0; i<1; i++){//<Любой текст на ваш выбор с количеством строк равным последним двум цифрам номера зачетки>
		strcat(testtxt_str, "1\n");//Записываем в массив символы 1\n (чтобы получилась 1 строка с любым текстом. в данном случае - 1 строка с цифрой 1.
	}
	Link text = createNode("test.txt", testtxt_str, S_IFREG | 0556);//Создаем виртуальный файл test.txt
